...
use crate::{error::GenericError, get_competitions_in_fantasy_tournament, get_fantasy_tournament, get_user_participants_in_tournament};
use crate::query::get_fantasy_tournament_model;

pub async fn is_user_allowed_to_exchange(db: &impl ConnectionTrait, user_id: i32, tournament_id: i32) -> Result<bool, GenericError> {
...
pub async fn see_which_users_can_exchange(db: &impl ConnectionTrait, tournament: &entity::fantasy_tournament::Model) -> Result<Vec<crate::dto::User>, GenericError> {
    ...
    while let Some(exchange_time) = possible_exchange_window_time {
        ...
        if exchange_time.hour() == 20 {
            possible_exchange_window_time = exchange_time.add(Duration::days(1)).with_hour(8).and_then(|x|x.with_minute(0).and_then(|x|x.with_second(0)))
        } else {
...
async fn get_first_exchange_window_time(db: &impl ConnectionTrait, tournament: &entity::fantasy_tournament::Model) -> Result<Option<NaiveDateTime>, GenericError>  {
    ...
    if comps.iter().any(|c|c.status==CompetitionStatus::Running) {
        ...
        let first_exchange_time: Option<NaiveDateTime> = end_time.add(Duration::days(1)).naive_local().with_hour(8).and_then(|x|x.with_minute(0).and_then(|x|x.with_second(0)));
...
async fn last_possible_exchange_window_time(db: &impl ConnectionTrait, tournament: &entity::fantasy_tournament::Model) -> Result<Option<NaiveDateTime>, GenericError> {
   Ok(next_competition_beginning(db, tournament).await?.map(|x|{
         x.add(Duration::days(-1)).and_time(NaiveTime::from_hms_opt(20,0,0).unwrap())
   }))
}

async fn next_competition_beginning(db: &impl ConnectionTrait, tournament: &entity::fantasy_tournament::Model) -> Result<Option<NaiveDate>, GenericError> {
    let comps = get_competitions_in_fantasy_tournament(db, tournament.id).await?;
    let next_competition = comps.iter().filter(|comp|comp.status==CompetitionStatus::NotStarted).min_by_key(|c|c.start_date);
